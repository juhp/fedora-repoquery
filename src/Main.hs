...
queryOptions =
  ...
  <|> dnfOption'' "wd" "whatdepends"
  <|> dnfOption "whatenhances"
  <|> dnfOption "whatobsoletes"
  <|> dnfOption'' "wp" "whatprovides"
  <|> dnfOption "whatrecommends"
  <|> dnfOption'' "wr" "whatrequires"
...
runMain :: Arch -> Bool -> Verbosity -> Bool -> Bool -> RepoSource
        -> Maybe AllReleases -> [Arch] -> Bool -> Bool -> Command -> IO ()
runMain sysarch dnf4 verbose dynredir time reposource mallreleases archs testing debug command = do
  case command of
    CacheSize -> cacheSize
